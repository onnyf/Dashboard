    ...
    <div className="bg-[#EEF2F1] min-h-screen pt-[30px] px-4 md:pl-[268px] md:pr-6 pb-6 space-y-6">
        ...
        {[
          ...
          return (
            <div
              ...
              style={{ height: "107px", boxShadow: "0px 10px 18px -2px #10192812" }}
            >
              <div className="p-4 flex flex-col justify-between h-full">
                ...
                <div className="flex justify-between items-center mt-1">
                  ...
                  <div
                    className={`flex items-center gap-1 text-[12px] font-medium px-2 py-[2px] rounded-full ${
                      isActive ? "bg-white text-[#12B76A]" : "bg-[#12B76A] text-white"
                    }`}
                  >
                ...
                <div className={`text-xs mt-1 ${isActive ? "text-white/70" : "text-[#8D98A4]"}`}>
                  From 0% (last 4 weeks)
                </div>
              </div>
            </div>
      ...
      <div className="bg-white shadow-sm rounded-xl ">
        <table className="min-w-full text-sm text-left  ">
          <thead className="bg-[#f5f7f7] h-[40px] text-[#4B5563]">
            <tr className="text-sm font-medium bg-[#FAFBFB]">
              ...
              <th className="py-2 px-4 text-right relative">
                <div onClick={() => setShowDropdown(!showDropdown)} className="flex items-center gap-1 justify-end cursor-pointer">
                ...
                {showDropdown && (
                  ...
                  >
                    <p className="text-[#8D98A4] text-[12px] font-medium mb-2 mr-5">Status type</p>
            ...
            {paginatedOrders.map((order, index) =>
              order.descriptions.map((desc, i) => (
                <tr key={`${index}-${i}`} className="border-b border-gray-100 hover:bg-gray-50">
                  {i === 0 && (
                    <>
                      <td rowSpan={order.descriptions.length} className="py-4 px-4 align-top font-medium text-sm text-gray-600">
                        {String((currentPage - 1) * itemsPerPage + index + 1).padStart(2, "0")}
                      </td>
                      <td rowSpan={order.descriptions.length} className="py-4 px-4 align-top">{order.id}</td>
                    </>
                  )}
                  <td className="py-2 px-4 align-top">{desc}</td>
                  {i === 0 && (
                    <>
                      <td rowSpan={order.descriptions.length} className="py-4 px-4 align-top">{order.investor}</td>
                      <td rowSpan={order.descriptions.length} className="py-4 px-4 align-top">{order.amount}</td>
                      <td rowSpan={order.descriptions.length} className="py-4 px-4 align-top">{order.date}</td>
                      <td rowSpan={order.descriptions.length} className="py-4 px-4 text-right align-top">
                ...
                return (
                  <button
                    ...
                    className={`px-2 py-1 rounded ${
                      currentPage === page ? "bg-green-800 text-white" : "hover:bg-gray-100"
                    }`}
                  >
                    {page}
                  </button>
